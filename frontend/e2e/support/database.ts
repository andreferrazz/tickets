...
import pg from 'pg';
import { ADMIN, DRAFT_ORG, MEMBER, OTHER_ORG, SEEDED_EVENTS, SESSIONS } from './fixtures';
...
const SEEDED_TABLES = ['sessions', 'events', 'organization_memberships', 'organizations', 'users'];
		...
		await client.query(
			`insert into events
			   (id, title, description, location, starts_at, status, organization_id,
			    created_by_id, seat_selection_enabled, inserted_at, updated_at)
			 values ($1, $2, 'seeded by the e2e run', 'Sao Paulo', $3, $4, $5, $6, false,
			         now() at time zone 'utc', now() at time zone 'utc')`,
